...
```{r}
df[df$height > 180 , "name"] #所有身高高於180的名字 
```

```{r}
subset(df, height > 170 & !isMale)$name # 身高高於170的女生名字
```
...
```{r}
df$height[!df$isMale] %>% mean #女生的平均身高
```

```{r}
sum( subset(df, !isMale)$noBuy ) # 女生的購買次數
```

```{r}
subset(df, skin_color == "white" & !isMale ) %>% nrow # 白皮膚女性的人數
```

```{r}
sum(df$skin_color == "white" & !df$isMale ) # 白皮膚女性的人數
```
