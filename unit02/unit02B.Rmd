...
```{r}
table(USDA$HighSodium) #計算USDA中是不是高鹽食物的數量
```
...
```{r}
tapply(USDA$Iron, USDA$HighProtein, mean, na.rm=TRUE) #在USDA中根據蛋白質統計鐵質平均量,並且忽略遺漏值
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, max, na.rm=TRUE)
#在USDA中根據碳水化合物統計維他命C最大量,並且忽略遺漏值
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, summary, na.rm=TRUE)
#在USDA中根據碳水化合物檢視維他命C的統計資料,並且忽略遺漏值
```
<br>
