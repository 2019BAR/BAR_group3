...
```
A: American Indian, Multiracial, Black, White 
...
```
A: The Married variable being missing is related to the Age value for the interviewee.
<br>
...
```{r}
tapply(A$Education=='No high school diploma', A$MetroArea, mean, na.rm=TRUE) %>% 
  sort %>% head
```
A: Iowa City, IA 
...
```{r}
tapply(A$Country=="India", A$MetroArea, sum, na.rm=TRUE) %>% sort %>% tail
```
...
In Brazil?
```{r}
tapply(A$Country=="Brazil", A$MetroArea, sum, na.rm=TRUE) %>% sort %>% tail
```
...
In Somalia?
```{r}
tapply(A$Country=="Somalia", A$MetroArea, sum, na.rm=TRUE) %>% sort %>% tail
```
